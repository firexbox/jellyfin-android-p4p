...
class ApiClientController(
     ...
     */
    suspend fun setupServer(hostname: String, isIp4p: Boolean = false) {
        appPreferences.currentServerId = withContext(Dispatchers.IO) {
            serverDao.getServerByHostname(hostname)?.id ?: serverDao.insert(hostname, isIp4p)
        }
        // Resolve IP4P address to a connectable URL before setting the API base URL
        val baseUrl = if (isIp4p) resolveIp4pUrl(hostname) else hostname
    ...
    fun getApiClient(server: Long, user: Long): ApiClient {
        ...
        val baseUrl = if (serverUser.server.isIp4p) {
            // Best-effort sync resolution for IP4P (DNS resolution not available here)
